...
function gsData = LocalParseArgs(varargin) 
    ...
    for i = 1 : 2 : length(varargin)-1 
        ...
        switch(lower(param_arg)) 
            ...
            case 'gsdevicewidthpoints'
               
               if ~isnumeric(param_value) 
                  warning('ps2pdf:deviceheightpoints', ...
                        '''gsdeviceheightpoints'' value <%s> should be numeric, ignoring it', param_value, class(param_value));
               else
                  gsData.deviceWidthPoints = param_value;
               end
            
            case 'gsdeviceheightpoints'
               if ~isnumeric(param_value)
                  warning('ps2pdf:deviceheightpoints', ...
                        '''deviceheightpoints'' value <%s> should be numeric, ignoring it', param_value, class(param_value));
               else
                  gsData.deviceHeightPoints = param_value;
               end
...
function gsData = LocalCreateResponseFile(gsData) 
   ...
   if isfield(gsData, 'deviceWidthPoints') 
      fprintf( rsp_fid, '-dDEVICEWIDTHPOINTS=%d\n', gsData.deviceWidthPoints );
   end
   if isfield(gsData, 'deviceHeightPoints') 
      fprintf( rsp_fid, '-dDEVICEHEIGHTPOINTS=%d\n', gsData.deviceHeightPoints );
   end
