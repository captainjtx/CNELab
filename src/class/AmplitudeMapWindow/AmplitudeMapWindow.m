classdef AmplitudeMapWindow < handle
    ...
    methods
        ...
        function varinitial(obj)
            ...
            obj.smooth_row_ = 4;
            obj.smooth_col_ = 4;
            obj.export_movie_win=ExportAmpMovieWindow(obj);
        end
        ...
        function buildfig(obj)
            ...
            if strcmp(obj.interp_scatter, 'interp')
                set(obj.map_interp_menu, 'check', 'on');
                set(obj.map_scatter_menu, 'check', 'off');
            elseif strcmp(obj.interp_scatter, 'scatter')
                set(obj.map_interp_menu, 'check', 'off');
                set(obj.map_scatter_menu, 'check', 'on');
            end
...
%             obj.JSpeedSpinner.getModel().setStepSize((1,java.lang.Integer(val/5)));
        ...
        function PlayAmplitudeMap(obj)
            nt=obj.bsp.VideoLineTime+obj.JStepSpinner.getValue()/obj.fs;
            
            if nt > obj.bsp.TotalTime
                obj.PausePlay();
            end
            nt = min(max(0, nt), obj.bsp.TotalTime);
            obj.bsp.VideoLineTime = nt;
