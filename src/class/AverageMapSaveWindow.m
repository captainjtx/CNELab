classdef AverageMapSaveWindow<handle
    ...
    methods
        function obj=AverageMapSaveWindow(p)
        ...
        function fname=auto_file_name(obj)
            fname=['c_',num2str(obj.parent.min_clim),'_',num2str(obj.parent.max_clim)];
        end
        ...
        function ExportCallback(obj)
            ...
            for i=1:length(obj.parent.SpatialMapFig)
                waitbar(i/length(obj.parent.SpatialMapFig));
                figname=get(obj.parent.SpatialMapFig(i),'Name');
                fname=fullfile(obj.dest_dir,[obj.filename,'_',figname,'_',obj.auto_file_name]);
                
                set(obj.parent.SpatialMapFig(i),'color','white');
                export_fig(obj.parent.SpatialMapFig(i),['-',pic_format],'-nocrop','-opengl',['-r',num2str(obj.res_ppi)],fname);
            end
            close(wait_bar_h);
        end
