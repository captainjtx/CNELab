...
if ~user_string('ghostscript', path)
    warning('export_fig:gostscript','Path to ghostscript installation could not be saved. Enter it manually in ghostscript.txt.');
    return
end
return
...
% Check the path is valid
[good,~] = system(sprintf('"%s" -h', path)); % suppress output
good = ~good;
return