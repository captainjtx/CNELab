...
for i=1:length(listings)
    for target=1:2
        ...
        switch target
            case 1
                targetind=columnData{12}==-1;
            case 2
                targetind=columnData{12}>0;
        end
        
        if fnameInfo{1}==3&&fnameInfo{3}==1&&strcmpi(fnameInfo{2}{:},'medicineoff')
            textCol=find((columnData{1}==fnameInfo{1})&strcmpi(columnData{2},fnameInfo{2})&...
                (columnData{3}==fnameInfo{3})&(columnData{end}==0)&(columnData{4}~=41)&targetind);
        else
            textCol=find((columnData{1}==fnameInfo{1})&strcmpi(columnData{2},fnameInfo{2})&...
                (columnData{3}==fnameInfo{3})&(columnData{end}==0)&targetind);
        end
        ...
        save([['Pt', num2str(fnameInfo{1}),' ',fnameInfo{2}{:},' Session',num2str(fnameInfo{3}),' TargetNum',num2str(target)],'.mat'],'-struct','tfmat');
    end
end