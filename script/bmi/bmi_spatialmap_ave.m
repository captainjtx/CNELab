...
gauss_interpolate_120_grid(a,pwRest,channelindex,30,30,pn,true,plotmissing);
...
for m=1:length(movements)
    ...
    gauss_interpolate_120_grid(a,coordinates(:,3),channelindex,30,30,pn,scale,plotmissing,percent_threshold);
    cb=colorbar('Units','Pixels');
    cbpos=get(cb,'Position');
    set(a,'Position',[dw/2,dh/2,12*dw,10*dh]);
    set(cb,'Position',[13*dw,dh/2,cbpos(3),cbpos(4)])
