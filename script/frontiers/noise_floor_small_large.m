...
rfb_small = sqrt(baseline_nf_small(:)/200*fs/2/nfft*fs); %rms in uV
rfb_large = sqrt(baseline_nf_large(:)/200*fs/2/nfft*fs);

figure
set(gcf,'position',[0,0,250,380]);

bp=boxplot(cat(1,...
    rfb_small,...
    rfb_large),...
    contact_4,'labelverbosity','all','labelorientation','horizontal',...
    'OutlierSize',10,'positions',[1,2]);
hold on
scatter([1, 2],[mean(rfb_small), mean(rfb_large)], 50, '*r')
ylabel('RMS (uV)')
set(gca,'fontsize',16);
...
%%
[p1, h1] = ttest2(rfb_small, rfb_large, 'tail', 'both')