...
% lfb_small = move_lfb_small(:)./(move_nf_small(:)/200*(lfb(2)-lfb(1)));
% lfb_large = move_lfb_large(:)./(move_nf_large(:)/200*(lfb(2)-lfb(1)));
% hfb_small = move_hfb_small(:)./(move_nf_small(:)/200*(hfb(2)-hfb(1)));
% hfb_large = move_hfb_large(:)./(move_nf_large(:)/200*(hfb(2)-hfb(1)));
% ufb_small = move_ufb_small(:)./(move_nf_small(:)/200*(ufb(2)-ufb(1)));
% ufb_large = move_ufb_large(:)./(move_nf_large(:)/200*(ufb(2)-ufb(1)));
...
% lfb_small = baseline_lfb_small(:)./(baseline_nf_small(:)/200*(lfb(2)-lfb(1)));
% lfb_large = baseline_lfb_large(:)./(baseline_nf_large(:)/200*(lfb(2)-lfb(1)));
% hfb_small = baseline_hfb_small(:)./(baseline_nf_small(:)/200*(hfb(2)-hfb(1)));
% hfb_large = baseline_hfb_large(:)./(baseline_nf_large(:)/200*(hfb(2)-hfb(1)));
% ufb_small = baseline_ufb_small(:)./(baseline_nf_small(:)/200*(ufb(2)-ufb(1)));
% ufb_large = baseline_ufb_large(:)./(baseline_nf_large(:)/200*(ufb(2)-ufb(1)));
...
bp=boxplot(cat(1,...
    ...
    ufb_small,...
    ufb_large),...
    {cat(1, freq_2, freq_3),cat(1, contact_2, contact_3)},'labelverbosity','all','labelorientation','horizontal',...
    'OutlierSize',10,'positions',[1,2,3,4]);
hold on
scatter([1, 2, 3, 4],[mean(hfb_small), mean(hfb_large), mean(ufb_small), mean(ufb_large)], 50, '*r')
ylabel('SNR (dB)')
set(gca,'fontsize',16);
...
%%
[p1, h1] = ranksum(lfb_small, lfb_large, 'tail', 'both')
[p2, h2] = ranksum(hfb_small, hfb_large, 'tail', 'both')
[p3, h3] = ranksum(ufb_small, ufb_large, 'tail', 'both')
%%
[p1, h1] = ttest2(lfb_small, lfb_large, 'tail', 'both')
[p2, h2] = ttest2(hfb_small, hfb_large, 'tail', 'both')
[p3, h3] = ttest2(ufb_small, ufb_large, 'tail', 'both')