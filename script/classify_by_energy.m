...
%                     [trC,trO,tsC,tsO]=CSPFeatures(data_c(:,Chi,TrCI), data_o(:,Chi,TrOI), data_c(:,Chi,TsCI), data_o(:,Chi,TsOI), Nf, W);
                    ...
                    trC=squeeze(sum(sum(data_c(:,Chi,TrCI).^2,1),2));
                    trO=squeeze(sum(sum(data_o(:,Chi,TrOI).^2,1),2));
                    
                    tsC=squeeze(sum(sum(data_c(:,Chi,TsCI).^2,1),2));
                    tsO=squeeze(sum(sum(data_o(:,Chi,TsOI).^2,1),2));
