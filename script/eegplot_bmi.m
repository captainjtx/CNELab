% eegplot_bmi() - Scroll (horizontally and/or vertically) through multichannel data.
%             Allows vertical scrolling through channels and manual marking
%             and unmarking of data stretches or epochs for rejection.
% Usage:
%           >> eegplot_bmi(data, 'key1', value1 ...); % use interface buttons, etc.
%      else
%           >> eegplot_bmi('noui', data, 'key1', value1 ...); % no user interface;
%                                                         % use for plotting
% Menu items:
...
%                  button is pressed (see Outputs); called from pop_eegplot_bmi() or
...
%                  Called from command line, subsequent functions eegplot_bmi2event() or
%                  eegplot_bmi2trials() allow processing trials marked with different colors
...
%                  pop_eegplot_bmi(), the channel labels of the dataset will be used.
...
% Optional keywords:
%    'vlines'     - verticle lines (structure with fields, 'XData','LineStyle','Color')
...
%                   eegplot_bmi2event() and eegplot_bmi2trial()).
...
%    'winalpha'     alpha value for winrej patches
%    'events'     - [struct] EEGLAB event structure (EEG.event) to use to show events.
%    'eventalpha'     alpha value for winrej patches
...
%    'tag'        - [string] Matlab object tag to identify this eegplot_bmi() window (allows
%                    keeping track of several simultaneous eegplot_bmi() windows).
%    'children'   - [integer] Figure handle of a *dependent* eegplot_bmi() window. Scrolling
...
%                    this variable. (See eegplot_bmi2trial() and eegplot_bmi2event()).
%
% Author: Arnaud Delorme & Colin Humphries, CNL/Salk Institute, SCCN/INC/UCSD, 1998-2001
%
% See also: eeg_multieegplot_bmi(), eegplot_bmi2event(), eegplot_bmi2trial(), eeglab()
...
%
%  2:36 AM 12/2/2011
%  Ibrahim Onaran, adds vertical lines
%
%
...
function [outvar1] = eegplot_bmi(data, varargin) % p1,p2,p3,p4,p5,p6,p7,p8,p9)
...
YAXIS_NEG = 'off';                %#ok<NASGU> % 'off' = positive up
DEFAULT_NOUI_PLOT_COLOR = 'k';    %#ok<NASGU> % EEG line color for noui option
...
if ~ischar(data) % If NOT a 'noui' call or a callback from uicontrols
    ...
    try
        ...
        disp('eegplot_bmi() error: calling convention {''key'', value, ... } error: %s',errmsg.message); return;
    end;
    
    if ~isfield(g,'srate'), g.srate		= 256; 	end;
    if ~isfield(g,'vlines'), g.vlines		= []; 	end;
    if ~isfield(g,'spacing'), g.spacing	= 0; 	end;
    if ~isfield(g,'eloc_file'), g.eloc_file	= 0; 	end; % 0 mean numbered
    if ~isfield(g,'winlength'), g.winlength	= 5; 	end; % Number of seconds of EEG displayed
    if ~isfield(g,'position'), g.position	= ORIGINAL_POSITION; 	end;
    if ~isfield(g,'title'), g.title		= ['Scroll activity -- eegplot_bmi()']; 	end;
    if ~isfield(g,'trialstag'), g.trialstag	= -1; 	end;
    if ~isfield(g,'winrej'), g.winrej		= []; 	end;
    if ~isfield(g,'winalpha'), g.winalpha		= 0.3; 	end;
    if ~isfield(g,'command'), g.command	= ''; 	end;
    if ~isfield(g,'tag'), g.tag		= 'EEGPLOT'; end;
    if ~isfield(g,'xgrid'), g.xgrid		= 'off'; end;
    if ~isfield(g,'ygrid'), g.ygrid		= 'off'; end;
    if ~isfield(g,'color'), g.color		= 'off'; end;
    if ~isfield(g,'submean'), g.submean	= 'on'; end;
    if ~isfield(g,'children'), g.children	= 0; end;
    if ~isfield(g,'limits'), g.limits	    = [0 1000*(size(data,2)-1)/g.srate]; end;
    if ~isfield(g,'freqlimits'), g.freqlimits	= []; end;
    if ~isfield(g,'dispchans'), g.dispchans  = size(data,1); end;
    if ~isfield(g,'wincolor'), g.wincolor   = [ 0.8345 1 0.9560]; end;
    if ~isfield(g,'butlabel'), g.butlabel   = 'REJECT'; end;
    if ~isfield(g,'colmodif'), g.colmodif   = { g.wincolor }; end;
    if ~isfield(g,'scale'), g.scale      = 'on'; end;
    if ~isfield(g,'events'), g.events      = []; end;
    if ~isfield(g,'ploteventdur'), g.ploteventdur = 'on'; end;
    if ~isfield(g,'eventalpha'), g.eventalpha		= 0.1; 	end;
    if ~isfield(g,'data2'), g.data2      = []; end;
    if ~isfield(g,'plotdata2'), g.plotdata2 = 'off'; end;
    if ~isfield(g,'closecallback'), g.closecallback = @close_callback; end
    if ~isfield(g,'analysischannel'), g.analysischannel = 1; end
    if ~isfield(g,'plotdatacb'), g.plotdatacb = []; end
    if ~isfield(g,'tfmapcb'), g.tfmapcb = []; end
    ...
    for index=1:length(gfields)
        switch gfields{index}
            case {'spacing', 'srate' 'eloc_file' 'winlength' 'position' 'title' ...
                    ...
                    'ploteventdur' 'butlabel' 'scale' 'events' 'data2' 'plotdata2' 'vlines' ...
                    'winalpha' 'eventalpha','closecallback','analysischannel' 'plotdatacb' 'tfmapcb'};
            otherwise, error(['eegplot_lfp: unrecognized option: ''' gfields{index} '''' ]);
        end;
    end;
    ...
    if size(g.winrej,2) < 5+g.chans
        if size(g.winrej,2) < 5
            g.winrej = [g.winrej g.wincolor(ones(size(g.winrej,1),1),:)];
        end
        g.winrej = [g.winrej zeros(size(g.winrej,1),g.chans-size(g.winrej,2)+5)];
    elseif size(g.winrej,2) > 5+g.chans
        g.winrej(:,6+g.chans:end) = [];
    end
    
    figh = figure('UserData', g,... % store the settings here
        'Color',DEFAULT_FIG_COLOR, 'name', g.title,'CloseRequestFcn',g.closecallback,...
    ...
    q = [pos(1) pos(2) 0 0]; %#ok<NASGU>
    s = [pos(3) pos(4) pos(3) pos(4)]./100;%#ok<NASGU>
    clf;
    ...
    ax0 = axes('tag','backeeg','parent',figh,...
        ...
        'Box','off','xgrid','off', 'xaxislocation', 'top');%#ok<NASGU>
    ...
    if ischar(g.eloc_file) || isstruct(g.eloc_file)  % Read in electrode names
        ...
        eegplot_bmi('setelect', g.eloc_file, ax1);
    end;
    ...
    posbut(21,:) = [ 0.575    0.0254    0.0385    0.0339 ]; % Plot Data
    posbut(22,:) = [ 0.615    0.0254    0.0385    0.0339 ]; % Tf Map
    posbut(23,:) = [ 0.655    0.0254    0.0385    0.0339 ]; % Channel
    posbut(:,1) = posbut(:,1)+0.2;
    
    % Five move buttons: << < text > >> and one reject button
    ...
    u(1) = uicontrol('Parent',figh, ...
        ...
        'Tag','Pushbutton1',...
        'string','<<',...
        'Callback',{@eegplot_bmi,'drawp',1});
    u(2) = uicontrol('Parent',figh, ...
        'Units', 'normalized', ...
        'Position', posbut(2,:), ...
        'Tag','Pushbutton2',...
        'string','<',...
        'Callback',{@eegplot_bmi,'drawp',2});
    u(5) = uicontrol('Parent',figh, ...
        ...
        'Callback', {@eegplot_bmi,'drawp',0});
    u(3) = uicontrol('Parent',figh, ...
        'Units', 'normalized', ...
        'Position',posbut(3,:), ...
        'Tag','Pushbutton3',...
        'string','>',...
        'Callback',{@eegplot_bmi,'drawp',3});
    u(4) = uicontrol('Parent',figh, ...
        'Units', 'normalized', ...
        'Position',posbut(4,:), ...
        'Tag','Pushbutton4',...
        'string','>>',...
        'Callback',{@eegplot_bmi,'drawp',4});
    ...
    u(21) = uicontrol('Parent',figh, ...
        ...
        'Tag','Pushbutton21',...
        'string','Plot Data',...
        'Callback',{@eegplot_bmi,'plotdata',1});
    ...
    u(22) = uicontrol('Parent',figh, ...
        ...
        'Tag','Pushbutton22',...
        'string','TFMap',...
        'Callback',{@eegplot_bmi,'tfmapbutton',1});
    
    u(23) = uicontrol('Parent',figh, ...
        'Units', 'normalized', ...
        'Position', posbut(23,:), ...
        'Tag','ChannelIndex',...
        'string',num2str(g.analysischannel),...
        'Style','Edit','TooltipString','Analysis Channel Index');
    ...
    if ~isempty(g.command)
        u(12) = uicontrol('Parent',figh, ...
            ...
            'Position',posbut(12,:), ...
            'Tag','Accept',...
            'string',g.butlabel, 'callback', {@acceptcommand,tmpcom});
    end;
    u(13) = uicontrol('Parent',figh, ...
        'Units', 'normalized', ...
        'Position',posbut(13,:), ...
        'string',fastif(isempty(g.command),'CLOSE', 'CANCEL'), 'callback', @close_callback);
    
    if ~isempty(g.events)
        u(17) = uicontrol('Parent',figh, ...
            'Units', 'normalized', ...
            'Position',posbut(17,:), ...
            'string', 'Event types', 'callback', {@eegplot_bmi,'drawlegend',figh}');
    end;
    ...
    uimenu('Parent',m(7),'Label','Edit figure', 'Callback', {@eegplot_bmi,'noui'});
    uimenu('Parent',m(7),'Label','Accept and close', 'Callback', {@acceptcommand,tmpcom} );
    ...
    uimenu('Parent',m(11),'Label','Hide marks','Callback',@hidemarkscb);
    ...
    % Scale Eye %%%%%%%%%
    timestring = ['[OBJ1,FIG1] = gcbo;',...
        'eegplot_bmi(''scaleeye'',OBJ1,FIG1);',...
        'clear OBJ1 FIG1;'];
    m(7) = uimenu('Parent',m(1),'Label','Show scale','Callback',timestring);
    
    % Title %%%%%%%%%%%%
    uimenu('Parent',m(1),'Label','Title','Callback','eegplot_bmi(''title'')')
    ...
    uimenu('Parent',m(2),'Label','Time range to display',...
        'Callback','eegplot_bmi(''window'')')
    
    % Electrode window %%%%%%%%
    uimenu('Parent',m(2),'Label','Number of channels to display',...
        'Callback','eegplot_bmi(''winelec'')')
    ...
    uimenu('Parent',m(6),'Label','Load .loc(s) file',...
        'Callback','eegplot_bmi(''loadelect'');')
    ...
    uimenu('Parent',zm,'Label','Zoom on', 'callback', @commandzoom,'Tag','ZoomOn');
    uimenu('Parent',zm,'Label','Zoom off', 'separator', 'on', 'callback',@zoomoffcb);
    uimenu('Parent',figh,'Label', 'Help', 'callback', 'pophelp(''eegplot_bmi'');');
    
    % Events %%%%%%%%
    zm = uimenu('Parent',m(2),'Label','Events');
    complotevent = [ 'g = get(gcbf, ''userdata'');' ...
        'g.plotevent = ''on'';' ...
        'set(gcbf, ''userdata'', g); clear g; eegplot_bmi(''drawp'', 0);'];
    comnoevent   = [ 'g = get(gcbf, ''userdata'');' ...
        'g.plotevent = ''off'';' ...
        'set(gcbf, ''userdata'', g); clear g; eegplot_bmi(''drawp'', 0);'];
    comeventleg  = [ 'eegplot_bmi(''drawlegend'', gcbf);'];
    ...
    if ~isempty(g.events)
        if ischar(g.events(1).type)
            [g.eventtypes tmpind indexcolor] = unique({g.events.type}); %#ok<ASGLU> % indexcolor countinas the event type
        else [g.eventtypes tmpind indexcolor] = unique([ g.events.type ]); %#ok<ASGLU>
        end;
    ...
    set(figh, 'userdata', g,'CurrentAxes',ax1);
    % %%%%%%%%%%%%%%%%%%%%%%%%%%
    % Plot EEG Data
    % %%%%%%%%%%%%%%%%%%%%%%%%%%
    hold on
    ...
    eegplot_bmi('scaleeye', [], gcf);
    if strcmpi(g.scale, 'off')
        eegplot_bmi('scaleeye', 'off', gcf);
    end;
    
    eegplot_bmi('drawp', 0);
    eegplot_bmi('drawp', 0);
    if g.dispchans ~= g.chans
        eegplot_bmi('zoom', gcf);
    end;
    eegplot_bmi('scaleeye', [], gcf);
    ...
    try p1 = varargin{1}; p2 = varargin{2}; p3 = varargin{3}; catch, end; %#ok<CTCH>
    switch data
        case 'plotdata'
            g = get(gcbf, 'userdata');
            ax1 = findobj('tag','eegaxis','parent',gcbf);
            data = get(ax1,'UserData')';
            Ch = findobj(gcbf,'tag','ChannelIndex');
            WndIx = fix(g.winrej(end,1:2));
            Ch=str2double(get(Ch,'String')); 
            if isempty(g.plotdatacb)
                figure(2);
                [b,a] = butter(2,16/g.srate*2,'high');
                plot(filtfilt(b,a,normalise(data(WndIx(1):WndIx(2),min(end,Ch)))));
                ylim([-2 2]);
            else
                feval(g.plotdatacb,data,WndIx,Ch,g.srate);
            end
        case 'tfmapbutton'
            g = get(gcbf, 'userdata');
            ax1 = findobj('tag','eegaxis','parent',gcbf);
            ...
            if isempty(g.tfmapcb)
                %modify for BMI
                figure(3)
                [tf,t,f]=stft(data(WndIx(1):WndIx(2)),256,500,128,1,0);
                figure(3);
                imagesc(t,f,20*log10(tf+eps),[-10 5]);
                axis xy;
            else
                feval(g.tfmapcb,data,WndIx,Ch,g.srate);
            end
            
        case 'drawp' % Redraw EEG and change position
            
            % this test help to couple eegplot_bmi windows
            if exist('p3','var')
                figh = p3;
                figure(p3);
            else
            ...
            if g.trialstag(1) == -1
                g.time    = str2num(get(EPosition,'string')); %#ok<ST2NM>
            else
                g.time    = str2num(get(EPosition,'string')); %#ok<ST2NM>
                g.time    = g.time - 1;
            end;
            g.spacing = str2num(get(ESpacing,'string')); %#ok<ST2NM>
            ...
            if ~isempty(g.data2)
                switch lower(g.submean) % subtract the mean ?
                    case 'on', meandata = mean(g.data2(:,lowlim:highlim),2)';
                    case 'nan',meandata = nan_mean(g.data2(:,lowlim:highlim)');
                    otherwise, meandata = zeros(1,g.chans);
                end;
            else
                switch lower(g.submean) % subtract the mean ?
                    case 'on', meandata = mean(data(:,lowlim:highlim),2)';
            ...
            if g.children ~= 0
                ...
                eegplot_bmi( 'drawp', p1, p2, g.children);
                figure(figh);
            end;
            ...
            YLim = get(ax1,'Ylim');
            if ~isempty(g.vlines)
                for iv = 1:length(g.vlines)
                    xdat = g.vlines(iv).XData(:) - g.time*g.srate;
                    xdat = [xdat xdat xdat*NaN]';
                    ydat = xdat;
                    ydat(1,:) = YLim(1);
                    ydat(2,:) = YLim(2);
                    line(xdat(:),ydat(:),'Color',g.vlines(iv).Color,'LineStyle',g.vlines(iv).LineStyle);
                end
            end
        case 'drawb' % Draw background ******************************************************
            ...
            displaymenu = findobj('tag','displaymenu','parent',gcf); %#ok<NASGU>
            if ~isempty(g.winrej) && g.winstatus
                if g.trialstag ~= -1 % epoched data
                    ...
                    if ~isempty(indices)
                        ...
                        catch, [cumul indicescount] = myhistc(tmpwins1, (min(tmpwins1)-1):g.trialstag:max(tmpwins2)); %#ok<CTCH>
                        end;
                        ...
                        for tmpi = 1:length(tmpwins1)
                            ...
                            set(h, 'EdgeColor', get(h, 'facecolor'),'FaceAlpha',g.winalpha)
                        end;
                    end;
                else
            ...
            if strcmpi(g.plotevent, 'on')
                ...
                for index = 1:length(event2plot)
                    ...
                    tmph   = plot([ tmplat tmplat ], ylim, 'color', g.eventcolors{ event2plot(index) }, ...
                        ...
                        'linewidth', g.eventwidths( event2plot(index) ) ); %#ok<NASGU>
                    ...
                    if g.ploteventdur && ~isempty(g.eventlatencyend) ...
                            ...
                            &&  g.eventlatencyend(event2plot(index))-g.eventlatencies(event2plot(index)) > 1
                        ...
                        if tmplatend ~= 0,
                            ...
                            set(h, 'EdgeColor', get(h, 'facecolor'),'FaceAlpha',g.eventalpha)
                        end;
                    end;
                end;
            end;
            ...
            if g.trialstag(1) ~= -1
                ...
                [tmp indexdiv] = min(abs(nbdiv*divpossible-(g.limits(2)-g.limits(1)))); %#ok<ASGLU> % closest possible increment
                ...
                for i=1:length(alltag)-1
                    ...
                    if ~isempty(g.freqlimits)
                        tagpos  = [ tagpos linspace(alltag(i),alltag(i+1)-1, nbdiv) ]; %#ok<AGROW>
                    else
                        if tagzerooffset ~= 0
                        ...
                        tagpos  = [ tagpos [tmptagpos(end:-1:2) alltag(i)+tagzerooffset:incrementpoint:(alltag(i+1)-1)]]; %#ok<AGROW>
                    end;
                end;
        ...
        case 'draws'
            ...
            if g.trialstag(1) == -1
                ...
                g.time    = str2num(get(EPosition,'string'))-1; %#ok<ST2NM>
            end;
            g.spacing = str2num(get(ESpacing,'string')); %#ok<ST2NM>
        ...
        case 'winelec'  % change channel window size
            ...
            set(gcf, 'UserData', g);
            eegplot_bmi('updateslider', fig);
            eegplot_bmi('drawp',0);
            eegplot_bmi('scaleeye', [], fig);
            return;
        ...
        case 'loadelect' % load channels
            ...
            eegplot_bmi('setelect',[ inputpath inputname ],AXH0);
            return;
        ...
        case 'setelect'
            ...
            set(axeshand,'Fontsize',7);
        case 'title'
            % Get new title
            h = findobj('tag', 'eegplot_title');
            ...
            if ~isempty(h)
                ...
                if isempty(varargin)
                    result       = inputdlg2( { 'New title:' }, 'Change title', 1,  { '' }); %#ok<NASGU>
                else
                    result=varargin;
                    if ~isempty(result), h = textsc(result{1}, 'title'); set(h, 'tag', 'eegplot_title','Fontsize',16, 'Fontweight','Bold'); end;
                end
            end;
        ...
        case 'noui'
            ...
            obj = findobj(fig, 'style', 'text');    delete(obj);
        ...
        case 'zoom' % if zoom
            ...
            % ------------------
            if g.trialstag ~= -1
                Eposition = str2num(get(findobj('tag','EPosition','parent',fig), 'string')); %#ok<ST2NM>
                ...
                Eposition = str2num(get(findobj('tag','EPosition','parent',fig), 'string'))-1; %#ok<ST2NM>
            ...
            if exist('p2','var') == 1
                zoom on;
                set(gcbf, 'windowbuttondownfcn', [ 'zoom(gcbf,''down''); eegplot_bmi(''zoom'', gcbf, 1);' ]);
                set(gcbf, 'windowbuttonmotionfcn',@commandmove);
            end;
                ...
                for index = 1:nleg
                    ...
                    if iscell(g.eventtypes)
                        th=text(35, (index-0.5)*10, g.eventtypes{index}, ...
                            'color', g.eventtypecolors{index}); %#ok<NASGU>
                    else
        ...
        otherwise
            error(['Error - invalid eegplot_bmi() parameter: ',data])
    end
end
...
function acceptcommand(hobj,eventdata,tmpcom) %#ok<INUSL>
g = get(gcbf, 'userdata');
TMPREJ = g.winrej;
if g.children
    delete(g.children);
...
function zoomoffcb(hMenu,eventdata) %#ok<INUSD>
zoom(gcbf, 'off');
set(gcbf, 'windowbuttondownfcn', @commandpush);
set(gcbf, 'windowbuttonmotionfcn', @commandmove);
set(gcbf, 'windowbuttonupfcn', @commandrelease);

function commandpush(hobj,eventdata) %#ok<INUSD>
...
if g.incallback ~= 1 % interception of nestest calls
    ...
    set(gcbf,'UserData', g);
    eegplot_bmi('drawp', 0);  % redraw background
end


function commandzoom(hobj,event) %#ok<INUSD>
mnZoomOn = findobj(gcbf,'Tag','ZoomOn');

if hobj == mnZoomOn
    zoom(gcbf,'On');
else
    error('time and channel zooms not implemented');
end
set(gcbf, 'windowbuttondownfcn',@zoombdown);
set(gcbf, 'windowbuttonmotionfcn',@commandmove);

function zoombdown(hobj,eventdata) %#ok<INUSD>
zoom(gcbf,'down');
eegplot_bmi('zoom', gcbf, 1); ...
    

function commandmove(hobj,eventdata) %#ok<INUSD>
ax1 = findobj('tag','backeeg','parent',gcbf);
tmppos = get(ax1, 'currentpoint');
g = get(gcbf,'UserData');
if isstruct(g)      %check if we are dealing with the right window
    ...
    if g.incallback
        ...
        eegplot_bmidata = get(ax1, 'userdata');
        set(hh, 'string', num2str(eegplot_bmidata(g.chans+1-tmpelec, min(g.frames,max(1,double(round(tmppos(1)+lowlim)))))));  % put value in the box
    end
end

function commandrelease(hobj,eventdata) %#ok<INUSD>
ax1 = findobj('tag','backeeg','parent',gcbf); %#ok<NASGU>
g = get(gcbf,'UserData');
g.incallback = 0;
...
function eegslidercb(hobj,eventdata) %#ok<INUSD>
g = get(gcbf, 'userdata');
g.elecoffset = get(gcbo, 'value')*(g.chans-g.dispchans);
set(gcbf, 'userdata', g);
eegplot_bmi('drawp',0);


function hidemarkscb(hobj,eventdata) %#ok<INUSD>
g = get(gcbf, 'userdata');
if ~g.winstatus
...
g = get(gcbf,'userdata');
if ishandle(g.children) && g.children
    delete(g.children)
end
delete(gcbf)
