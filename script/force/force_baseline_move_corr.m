...
for i = 1:trNum
    squeeze_lfb(:, :, i) = filter_symmetric(b1, a1, squeeze(squeeze_data(:, ecog_ind, i)), 0, 0, 'iir');
    squeeze_hfb(:, :, i) = filter_symmetric(b2, a2, squeeze(squeeze_data(:, ecog_ind, i)), 0, 0, 'iir');
end
...
relax_hfb = [];
for i = 1:trNum
    relax_lfb(:, :, i) = filter_symmetric(b1, a1, squeeze(relax_data(:, ecog_ind, i)), 0, 0, 'iir');
    relax_hfb(:, :, i) = filter_symmetric(b2, a2, squeeze(relax_data(:, ecog_ind, i)), 0, 0, 'iir');
end
